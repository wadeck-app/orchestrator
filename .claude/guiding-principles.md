...
- ~~Sync dist changes to BOTH the local workspace AND the global npm install location - changes that only update one path appear to "not take effect".~~ **Withdrawn 2026-09-15.** Hand-copying `dist` into the global install is how you end up testing a local build while believing you tested the package: it produced a "verified" start-at-login that was broken in the published artefact, and it contradicts the rule above about testing with the installed `orch`. Publish through CI, `npm install -g`, then test. If a change must be tried before publishing, run it explicitly as `node packages/orchestrator-cli/bin/orch.js <cmd>` so the path makes clear it is not the installed one.
